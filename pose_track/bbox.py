...
from deepsort_opensource.YOLOv3 import YOLOv3
from deepsort_opensource.deep_sort import DeepSort
from deepsort_opensource.util import COLORS_10, draw_bboxes
...
class Detector(object):
    ...
    def detect(self):
        ...
        import pickle
        import json
        file_name = self.args.VIDEO_PATH.replace('_original', '').rsplit(".", 1)[0] + "_track"
        pickle.dump(bbox, open(file_name+'.pkl', "wb"))
        json.dump(bbox, open(file_name+'.json', "w"), sort_keys=True, indent=4, separators=(',', ': '))
